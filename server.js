  ...
  socket.on(
    ...
    (roomID, newParticipantName, socketID, peerID) => {
      ...
      socket.on(
        ...
        (response, newParticipantName, socketID, peerID) => {
          if (socket.id === oneRoom.hostId) {
            const tempSocketObj = oneRoom.tempParticipantSocket.shift();
      ...
      socket.on("media-update-ingoing", (mediaType, peerID) => {
        if (oneRoom.participantSocketID.includes(socket.id)) {
          socket.to(roomID).emit("media-update-outgoing", mediaType, peerID);
        }
      });
      socket.on("sending-message", (chatMessage, time) => {
        if (oneRoom.participantSocketID.includes(socket.id)) {
          socket.to(roomID).emit("receiving-message", chatMessage, time);
        }
      });
      socket.on("disconnect", () => {
        if (socket.id === oneRoom.hostId) {
          allUsersGlobally.splice(allUsersGlobally.indexOf(oneRoom), 1);
          ...
          socket.to(roomID).emit("participant-disconnected", socket.id);
        }
      });
    }
  );
