...
async function localVideoStart() {
  try {
    const [localVideoTrack, localAudioTrack] =
      await getLocalMediaStream(true, true)
...
presentScreenBtn.addEventListener('click', async () => {
  if (!presentScreenBtn.classList.contains('red')) {
    try {
      ...
      myScreenStream.addEventListener(
        'inactive',
        screenStreamInactive
      )
      const length = globalUsers.length
      for (let i = 1; i < length; i++) {
    ...
    myScreenStream.removeEventListener(
      'inactive',
      screenStreamInactive
    )
    myScreenStream = null
